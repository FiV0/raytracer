          ...
          (multiple-value-bind (lxr lyr lzr)
            ...
            (multiple-value-bind (fo fint) (first-hit light-source lxr lyr lzr)
              (when (and (not (null o)) (eq fo o) (point-equal fint int))
                (incf res (* (lambert o int lxr lyr lzr) (surface-color o))))))))
      res)))
  ...
  (let (surface hit dist)
    (dolist (o *world*)
      (let* ((h (intersect o pt xr yr zr)))
        (when (and h
;;                    (same-direction
;;                        (make-point :x xr :y yr :z zr)
;;                        (make-point :x (- (x h) (x pt))
;;                                    :y (- (y h) (y pt))
;;                                    :z (- (z h) (z pt))))
                   )
          (let ((d (distance h pt)))
...
(ray-test 5 "spheres_different_light.pgm")
